...
<body>
    ...
    <h1 class=" container text-end">Courses</h1>
    ...
    <main>
        <div class="card container">
            ...
            <div class="row justify-content-end align-items-end m-2">
                <a href="{{ route('courses.create') }}" class="btn btn-primary w-25">
                    Create Course
                </a>
            </div>
            ...
            <table class="table table-bordered">
                ...
                @foreach ($courses as $c)
                    <tr>
                        <td style="width:10%">{{ (string)$c->cid }}</td>
                        <td>{{ $c->cname }}</td>
                        <td style="width:15%">{{ $c->created_at }}</td>
                        <td style="width:15%"> {{ $c->updated_at }}</td>
                        <td style="width:15%">
                            <form action="{{ route('courses.destroy', $c->cid) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <div class="row justify-content-center align-items-center">
                                    <div class="col"style="margin-right:-15px">
                                        <a href="{{ route('courses.edit', $c->cid) }}"
